...
func parseCharacterResponse(resCharacter *CharacterResponse, resCharacterRelatedPerson *[]CharacterRelatedPersonResponse) *Character {
	...
	for _, info := range resCharacter.Infobox {
		switch info.Key {
		case "简体中文名":
			character.NameCN = kurohelperservice.GetStringValue(info.Value)
		case "别名":
			character.Aliases = parseAliases(info.Value)
		case "性别":
			character.Gender = kurohelperservice.GetStringValue(info.Value)
		case "生日":
			character.BirthDay = kurohelperservice.GetStringValue(info.Value)
		case "血型":
			character.BloodType = kurohelperservice.GetStringValue(info.Value)
		case "身高":
			character.Height = kurohelperservice.GetStringValue(info.Value)
		case "体重":
			character.Weight = kurohelperservice.GetStringValue(info.Value)
		case "BWH":
			character.BWH = kurohelperservice.GetStringValue(info.Value)
		case "年龄":
			character.Age = kurohelperservice.GetStringValue(info.Value)
		default:
			otherValue := kurohelperservice.GetStringValue(info.Value)
