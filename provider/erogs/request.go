...
func sendPostRequest(sql string) (string, error) {
	if !rateLimit(1) {
		return "", kurohelperservice.ErrRateLimit
	}
	...
	if os.Getenv("PROXY_USE") == "private" {
		dialer, err := proxy.GetProxyDialer(os.Getenv("PROXY_PRIVATE_IP"), os.Getenv("PROXY_PRIVATE_PORT"), nil)
		if err != nil {
			fmt.Print(err)
			return "", err
		}
		...
		dialer, err := proxy.GetProxyDialer(os.Getenv("PROXY_ADDRESS"), os.Getenv("PROXY_PORT"), proxyAuth)
		if err != nil {
			fmt.Print(err)
			return "", err
		}
	...
	if resp.StatusCode != 200 {
		return "", fmt.Errorf("%w %d", kurohelperservice.ErrStatusCodeAbnormal, resp.StatusCode)
	}
